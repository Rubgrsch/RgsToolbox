...
local function replyString()
	...
	for i=1, steps do
		local _, _, _, quantity, totalQuantity, _, _, _, _, _, _, _, isWeightedProgress = C_Scenario_GetCriteriaInfo(i)
		if isWeightedProgress then
			progress = quantity/totalQuantity
		else
	...
	return format("正在进行史诗钥石 +%d %s | 首领%d/%d | 进度%.0f%%", dungeonLevel, dungeonName, bosses, totalbosses, 100*progress)
end
...
local function SentMessage(target)
	...
	if not wipeTimer then
		wipeTimer = true
		C_Timer_After(30, WipePlayers)
	end
end

function frame:CHAT_MSG_WHISPER(_, _, sender, _, _, _, _, _, _, _, _, _, guid)
	if C_ChallengeMode_IsChallengeModeActive() and (C_BattleNet_GetGameAccountInfoByGUID(guid) or IsGuildMember(guid) or C_FriendList_IsFriend(guid)) and not sentPlayers[sender] then
		SentMessage(sender)
